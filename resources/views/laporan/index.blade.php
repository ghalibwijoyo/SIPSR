...
<div class="card mb-3 border-0 shadow-sm" id="filter-card">
    <div class="card-body">
        <form method="GET" action="{{ route('laporan.index') }}" class="row g-3" id="mainFilterForm">
            {{-- Smart Search Bar --}}
            <div class="col-12">
            ...
            <div class="col-12 mt-2">
                <div class="d-flex flex-wrap gap-2">
                    <a href="{{ route('laporan.index') }}" 
                       class="btn btn-sm {{ !request()->has('search') && !request()->has('category_id') && !request()->has('bank_id') && !request()->has('uploader_id') && !request()->has('tanggal_dari') && !request()->has('quick_filter') ? 'btn-success' : 'btn-outline-success' }}"
                       aria-label="Lihat semua dokumen">
                        Semua Dokumen
                    </a>
                    
                    {{-- System Quick Filters --}}
                    <a href="{{ route('laporan.index', array_merge(request()->except(['page']), ['quick_filter' => 'today'])) }}" 
                    ...
                    <a href="{{ route('laporan.index', array_merge(request()->except(['page']), ['quick_filter' => 'my_upload'])) }}" 
                    ...
                    <a href="{{ route('laporan.index', array_merge(request()->except(['page']), ['quick_filter' => 'pdf'])) }}" 
                    ...
                    @foreach($categories as $cat)
                        <a href="{{ route('laporan.index', array_merge(request()->except(['page']), ['category_id' => $cat->id])) }}" 
...
<div class="alert alert-secondary py-2 px-3 mb-3 d-flex justify-content-between align-items-center shadow-sm border-0">
    ...
    <a href="{{ route('laporan.index') }}" class="btn btn-sm btn-danger rounded-pill px-3">
...
<div class="offcanvas offcanvas-end" tabindex="-1" id="laporanAdvancedFilter" aria-labelledby="laporanAdvancedFilterLabel">
    ...
    <div class="offcanvas-body">
        <form method="GET" action="{{ route('laporan.index') }}" id="laporanAdvancedFilterForm">
            {{-- Preserve search if any --}}
            <input type="hidden" name="search" value="{{ request('search') }}">
            
            {{-- Category --}}
            <div class="mb-4">
                <label for="laporan_category_filter" class="form-label small fw-bold">Kategori Dokumen</label>
                <select name="category_id" id="laporan_category_filter" class="form-select" aria-label="Filter berdasarkan kategori dokumen">
            ...
            {{-- Bank --}}
            <div class="mb-4">
                <label for="laporan_bank_filter" class="form-label small fw-bold">Bank</label>
                <select name="bank_id" id="laporan_bank_filter" class="form-select" aria-label="Filter berdasarkan bank">
            ...
            {{-- Uploader --}}
            <div class="mb-4">
                <label for="laporan_uploader_filter" class="form-label small fw-bold">Pengunggah (Uploader)</label>
                <select name="uploader_id" id="laporan_uploader_filter" class="form-select" aria-label="Filter berdasarkan pengunggah dokumen">
            ...
            <div class="mb-4">
                ...
                <div class="row g-2 mb-3">
                    <div class="col-6">
                        <label for="laporan_tanggal_dari" class="form-label small text-muted mb-1">Dari Tanggal</label>
                        <input type="date" name="tanggal_dari" id="laporan_tanggal_dari" class="form-control form-control-sm" value="{{ request('tanggal_dari') }}" aria-label="Filter dokumen dari tanggal">
                    </div>
                    <div class="col-6">
                        <label for="laporan_tanggal_sampai" class="form-label small text-muted mb-1">Sampai Tanggal</label>
                        <input type="date" name="tanggal_sampai" id="laporan_tanggal_sampai" class="form-control form-control-sm" value="{{ request('tanggal_sampai') }}" aria-label="Filter dokumen sampai tanggal">
                    </div>
                </div>
                
                <small class="text-muted d-block mb-2">Quick preset:</small>
                <div class="d-flex flex-wrap gap-2">
                    <button type="button" class="btn btn-sm btn-outline-secondary py-1 px-2" onclick="setLaporanDateRange(0, 0); return false;" aria-label="Filter dokumen hari ini">Hari Ini</button>
                    <button type="button" class="btn btn-sm btn-outline-secondary py-1 px-2" onclick="setLaporanDateRange(7, 0); return false;" aria-label="Filter dokumen 1 minggu terakhir">1 Minggu</button>
                    <button type="button" class="btn btn-sm btn-outline-secondary py-1 px-2" onclick="setLaporanDateRange(30, 0); return false;" aria-label="Filter dokumen 1 bulan terakhir">1 Bulan</button>
                    <button type="button" class="btn btn-sm btn-outline-secondary py-1 px-2" onclick="setLaporanDateRange(365, 0); return false;" aria-label="Filter dokumen 1 tahun terakhir">1 Tahun</button>
                    <button type="button" class="btn btn-sm btn-outline-secondary py-1 px-2" onclick="setLaporanDateRange(1825, 0); return false;" aria-label="Filter dokumen 5 tahun terakhir">5 Tahun</button>
                </div>
            </div>
        </form>
    </div>
    <div class="offcanvas-footer p-3 border-top bg-light d-flex gap-2">
        <a href="{{ route('laporan.index') }}" class="btn btn-light flex-grow-1 border">
            <i class="bi bi-arrow-counterclockwise me-1"></i>Reset
        </a>
        <button type="submit" form="laporanAdvancedFilterForm" class="btn btn-success flex-grow-1">
